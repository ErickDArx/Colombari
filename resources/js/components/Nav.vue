...
<nav class="z-50 transition duration-500 ease-in-out shadow-xl container mx-auto px-10 dark:bg-gray-900 bg-gray-100 flex items-center justify-between flex-wrap bg-teal-500 p-6">
  ...
  <div v-click-outside="onClickOutside" class="flex items-center justify-center gap-1">
    <button @click="active = true" class="dark:text-gray-300 text-indigo-400 flex text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-indigo-400 transition duration-150 ease-in-out">
  ...
  <div v-show="active" class="z-50 origin-top-right absolute right-8 top-20 w-48 rounded-md shadow-lg">
    <div class="py-1 rounded-md dark:text-gray-300 dark:bg-gray-700 bg-white shadow-xs" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
      ...
      <a href="" @click.prevent="logout" class="flex items-center px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out" role="menuitem">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 dark:text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
